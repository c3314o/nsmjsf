...
auditStatusDataSource = new AuditStatusDataSource();
		...
		auditStatusWrapperList = AuditStatusAdapter
				.wrapAll(auditStatusList);
...
quarterDataSource = new QuarterDataSource();
	...
	private void init()
	{
		...
		if(this.editMode)
		{
			this.financialReport=financialReportDataSource.get(editId);
			  ...
			  this.selectedFiscalYearWrapper=FiscalYearAdapter.wrap(financialReport.getFiscalYear());
	
			
			
			  
			  this.selectedAuditStatusWrapper=AuditStatusAdapter.wrap(financialReport.getAuditStatus());
	
			
			
			  
			  this.selectedPostWrapper=PostAdapter.wrap(financialReport.getPost());
	
			
			
			  
			  this.selectedCompanyWrapper=CompanyAdapter.wrap(financialReport.getCompany());
	
			
			
			  
			  this.selectedQuarterWrapper=QuarterAdapter.wrap(financialReport.getQuarter());
	...
	public void setFinancialReportDataSource(FinancialReportDataSource financialReportDataSource) {
...
public List<Quarter> getQuarterList() {
	...
	public FinancialReport saveFinancialReport() {
		try {
			...
			financialReport.setFiscalYear(fiscalYear);
                  ...
                  AuditStatus auditStatus =selectedAuditStatusWrapper.getAuditStatus();
		...
		log.error(ex.getMessage());
			MessageService.error("Failed Saving FinancialReport .Try Again Later!");
			return null;
		}
	}
	...
	public void cancel()
	{
	    RequestContext.getCurrentInstance().closeDialog("createFinancialReport");
		
	}
	public FinancialReport saveFinancialReport(Session session){
	
	   this.financialReport= financialReportDataSource.create(this.financialReport,session);
