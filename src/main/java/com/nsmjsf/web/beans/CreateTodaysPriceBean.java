...
public class CreateTodaysPriceBean implements Serializable {
	...
	private void init() {
		...
		if (this.editMode) {
			...
			this.selectedCompanyWrapper = CompanyAdapter.wrap(todaysPrice
					.getCompany());
	...
	public void setTodaysPriceDataSource(
	...
	public TodaysPrice saveTodaysPrice(Session session) {
		...
		this.todaysPrice = todaysPriceDataSource.create(this.todaysPrice,
				session);
