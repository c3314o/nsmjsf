...
public class FinancialReportWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(FinancialReportWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			FinancialReportDataSource financialReportDataSource = new FinancialReportDataSource();
			FinancialReport financialReport = financialReportDataSource.get(Integer.parseInt(value));
			FinancialReportWrapper financialReportWrapper=FinancialReportAdapter.wrap(financialReport);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((FinancialReportWrapper) object).getFinancialReport().getFinancialReportId());
		} else {
			return null;
		}
	}
