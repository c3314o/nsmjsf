...
public class MapPostCompanyWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(MapPostCompanyWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			MapPostCompanyDataSource mapPostCompanyDataSource = new MapPostCompanyDataSource();
			MapPostCompany mapPostCompany = mapPostCompanyDataSource.get(Integer.parseInt(value));
			MapPostCompanyWrapper mapPostCompanyWrapper=MapPostCompanyAdapter.wrap(mapPostCompany);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((MapPostCompanyWrapper) object).getMapPostCompany().getMapPostCompanyId());
		} else {
			return null;
		}
	}
