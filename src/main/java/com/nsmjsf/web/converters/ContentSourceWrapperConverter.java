...
public class ContentSourceWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(ContentSourceWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			ContentSourceDataSource contentSourceDataSource = new ContentSourceDataSource();
			ContentSource contentSource = contentSourceDataSource.get(Integer.parseInt(value));
			ContentSourceWrapper contentSourceWrapper=ContentSourceAdapter.wrap(contentSource);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((ContentSourceWrapper) object).getContentSource().getContentSourceId());
		} else {
			return null;
		}
	}
