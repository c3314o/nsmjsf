...
public class ForumThreadWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(ForumThreadWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			ForumThreadDataSource forumThreadDataSource = new ForumThreadDataSource();
			ForumThread forumThread = forumThreadDataSource.get(Integer
					.parseInt(value));
			ForumThreadWrapper forumThreadWrapper = ForumThreadAdapter
					.wrap(forumThread);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((ForumThreadWrapper) object)
