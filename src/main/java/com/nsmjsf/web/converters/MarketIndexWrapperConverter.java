...
public class MarketIndexWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(MarketIndexWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			MarketIndexDataSource marketIndexDataSource = new MarketIndexDataSource();
			MarketIndex marketIndex = marketIndexDataSource.get(Integer.parseInt(value));
			MarketIndexWrapper marketIndexWrapper=MarketIndexAdapter.wrap(marketIndex);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((MarketIndexWrapper) object).getMarketIndex().getMarketIndexId());
		} else {
			return null;
		}
	}
