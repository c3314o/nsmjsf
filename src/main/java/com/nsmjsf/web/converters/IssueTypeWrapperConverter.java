...
public class IssueTypeWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(IssueTypeWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			IssueTypeDataSource issueTypeDataSource = new IssueTypeDataSource();
			IssueType issueType = issueTypeDataSource.get(Integer
					.parseInt(value));
			IssueTypeWrapper issueTypeWrapper = IssueTypeAdapter
					.wrap(issueType);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((IssueTypeWrapper) object).getIssueType()
					.getIssueTypeId());
		} else {
			return null;
		}
