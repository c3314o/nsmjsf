...
public class NotificationWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(NotificationWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			NotificationDataSource notificationDataSource = new NotificationDataSource();
			Notification notification = notificationDataSource.get(Integer
					.parseInt(value));
			NotificationWrapper notificationWrapper = NotificationAdapter
					.wrap(notification);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((NotificationWrapper) object)
