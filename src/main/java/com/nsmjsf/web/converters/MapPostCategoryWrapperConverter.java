...
public class MapPostCategoryWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(MapPostCategoryWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			MapPostCategoryDataSource mapPostCategoryDataSource = new MapPostCategoryDataSource();
			MapPostCategory mapPostCategory = mapPostCategoryDataSource.get(Integer.parseInt(value));
			MapPostCategoryWrapper mapPostCategoryWrapper=MapPostCategoryAdapter.wrap(mapPostCategory);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((MapPostCategoryWrapper) object).getMapPostCategory().getMapPostCategoryId());
		} else {
			return null;
		}
	}
