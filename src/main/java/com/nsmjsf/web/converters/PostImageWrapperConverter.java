...
public class PostImageWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(PostImageWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			PostImageDataSource postImageDataSource = new PostImageDataSource();
			PostImage postImage = postImageDataSource.get(Integer.parseInt(value));
			PostImageWrapper postImageWrapper=PostImageAdapter.wrap(postImage);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((PostImageWrapper) object).getPostImage().getPostImageId());
		} else {
			return null;
		}
	}
