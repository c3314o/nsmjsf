...
public class EnergyTypeWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(EnergyTypeWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			EnergyTypeDataSource energyTypeDataSource = new EnergyTypeDataSource();
			EnergyType energyType = energyTypeDataSource.get(Integer
					.parseInt(value));
			EnergyTypeWrapper energyTypeWrapper = EnergyTypeAdapter
					.wrap(energyType);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((EnergyTypeWrapper) object).getEnergyType()
					.getEnergyTypeId());
		} else {
			return null;
		}
