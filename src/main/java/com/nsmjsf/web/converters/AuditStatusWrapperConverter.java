...
public class AuditStatusWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(AuditStatusWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			AuditStatusDataSource auditStatusDataSource = new AuditStatusDataSource();
			AuditStatus auditStatus = auditStatusDataSource.get(Integer
					.parseInt(value));
			AuditStatusWrapper auditStatusWrapper = AuditStatusAdapter
					.wrap(auditStatus);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((AuditStatusWrapper) object)
					.getAuditStatus().getAuditStatusId());
		} else {
			return null;
		}
	}
