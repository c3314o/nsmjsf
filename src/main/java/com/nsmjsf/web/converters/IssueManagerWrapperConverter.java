...
public class IssueManagerWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(IssueManagerWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			IssueManagerDataSource issueManagerDataSource = new IssueManagerDataSource();
			IssueManager issueManager = issueManagerDataSource.get(Integer
					.parseInt(value));
			IssueManagerWrapper issueManagerWrapper = IssueManagerAdapter
					.wrap(issueManager);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((IssueManagerWrapper) object)
