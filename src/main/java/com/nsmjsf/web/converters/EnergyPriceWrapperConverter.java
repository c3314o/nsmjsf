...
public class EnergyPriceWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(EnergyPriceWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			EnergyPriceDataSource energyPriceDataSource = new EnergyPriceDataSource();
			EnergyPrice energyPrice = energyPriceDataSource.get(Integer
					.parseInt(value));
			EnergyPriceWrapper energyPriceWrapper = EnergyPriceAdapter
					.wrap(energyPrice);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((EnergyPriceWrapper) object)
					.getEnergyPrice().getEnergyPriceId());
		} else {
			return null;
		}
	}
