...
public class UserEnergySalesWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(UserEnergySalesWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserEnergySalesDataSource userEnergySalesDataSource = new UserEnergySalesDataSource();
			UserEnergySales userEnergySales = userEnergySalesDataSource.get(Integer.parseInt(value));
			UserEnergySalesWrapper userEnergySalesWrapper=UserEnergySalesAdapter.wrap(userEnergySales);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserEnergySalesWrapper) object).getUserEnergySales().getUserEnergySalesId());
		} else {
			return null;
		}
	}
