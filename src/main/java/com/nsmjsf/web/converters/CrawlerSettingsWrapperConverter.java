...
public class CrawlerSettingsWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(CrawlerSettingsWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			CrawlerSettingsDataSource crawlerSettingsDataSource = new CrawlerSettingsDataSource();
			CrawlerSettings crawlerSettings = crawlerSettingsDataSource.get(Integer.parseInt(value));
			CrawlerSettingsWrapper crawlerSettingsWrapper=CrawlerSettingsAdapter.wrap(crawlerSettings);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((CrawlerSettingsWrapper) object).getCrawlerSettings().getCrawlerSettingsId());
		} else {
			return null;
		}
	}
