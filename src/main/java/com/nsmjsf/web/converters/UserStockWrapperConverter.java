...
public class UserStockWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(UserStockWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserStockDataSource userStockDataSource = new UserStockDataSource();
			UserStock userStock = userStockDataSource.get(Integer.parseInt(value));
			UserStockWrapper userStockWrapper=UserStockAdapter.wrap(userStock);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserStockWrapper) object).getUserStock().getUserStockId());
		} else {
			return null;
		}
	}
