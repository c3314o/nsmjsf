...
public class UserWatchListWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(UserWatchListWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserWatchListDataSource userWatchListDataSource = new UserWatchListDataSource();
			UserWatchList userWatchList = userWatchListDataSource.get(Integer
					.parseInt(value));
			UserWatchListWrapper userWatchListWrapper = UserWatchListAdapter
					.wrap(userWatchList);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserWatchListWrapper) object)
					.getUserWatchList().getUserWatchListId());
		} else {
			return null;
		}
	}
