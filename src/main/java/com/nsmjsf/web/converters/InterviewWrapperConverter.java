...
public class InterviewWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(InterviewWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			InterviewDataSource interviewDataSource = new InterviewDataSource();
			Interview interview = interviewDataSource.get(Integer.parseInt(value));
			InterviewWrapper interviewWrapper=InterviewAdapter.wrap(interview);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((InterviewWrapper) object).getInterview().getInterviewId());
		} else {
			return null;
		}
	}
