...
public class UserEnergyWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(UserEnergyWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserEnergyDataSource userEnergyDataSource = new UserEnergyDataSource();
			UserEnergy userEnergy = userEnergyDataSource.get(Integer.parseInt(value));
			UserEnergyWrapper userEnergyWrapper=UserEnergyAdapter.wrap(userEnergy);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserEnergyWrapper) object).getUserEnergy().getUserEnergyId());
		} else {
			return null;
		}
	}
