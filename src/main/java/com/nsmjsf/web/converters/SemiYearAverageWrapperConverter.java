...
public class SemiYearAverageWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(SemiYearAverageWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			SemiYearAverageDataSource semiYearAverageDataSource = new SemiYearAverageDataSource();
			SemiYearAverage semiYearAverage = semiYearAverageDataSource
					.get(Integer.parseInt(value));
			SemiYearAverageWrapper semiYearAverageWrapper = SemiYearAverageAdapter
					.wrap(semiYearAverage);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((SemiYearAverageWrapper) object)
