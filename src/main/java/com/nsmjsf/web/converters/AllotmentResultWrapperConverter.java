...
public class AllotmentResultWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(AllotmentResultWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			AllotmentResultDataSource allotmentResultDataSource = new AllotmentResultDataSource();
			AllotmentResult allotmentResult = allotmentResultDataSource
					.get(Integer.parseInt(value));
			AllotmentResultWrapper allotmentResultWrapper = AllotmentResultAdapter
					.wrap(allotmentResult);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((AllotmentResultWrapper) object)
					.getAllotmentResult().getAllotmentResultId());
		} else {
			return null;
		}
	}
