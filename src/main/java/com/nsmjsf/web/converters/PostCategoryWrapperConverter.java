...
public class PostCategoryWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(PostCategoryWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			PostCategoryDataSource postCategoryDataSource = new PostCategoryDataSource();
			PostCategory postCategory = postCategoryDataSource.get(Integer.parseInt(value));
			PostCategoryWrapper postCategoryWrapper=PostCategoryAdapter.wrap(postCategory);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((PostCategoryWrapper) object).getPostCategory().getPostCategoryId());
		} else {
			return null;
		}
	}
