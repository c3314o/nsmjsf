...
public class TodaysPriceWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(TodaysPriceWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			TodaysPriceDataSource todaysPriceDataSource = new TodaysPriceDataSource();
			TodaysPrice todaysPrice = todaysPriceDataSource.get(Integer.parseInt(value));
			TodaysPriceWrapper todaysPriceWrapper=TodaysPriceAdapter.wrap(todaysPrice);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((TodaysPriceWrapper) object).getTodaysPrice().getTodaysPriceId());
		} else {
			return null;
		}
	}
