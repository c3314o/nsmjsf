...
public class CompanyDetailWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(CompanyDetailWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			CompanyDetailDataSource companyDetailDataSource = new CompanyDetailDataSource();
			CompanyDetail companyDetail = companyDetailDataSource.get(Integer
					.parseInt(value));
			CompanyDetailWrapper companyDetailWrapper = CompanyDetailAdapter
					.wrap(companyDetail);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((CompanyDetailWrapper) object)
					.getCompanyDetail().getCompanyDetailId());
		} else {
			return null;
		}
	}
