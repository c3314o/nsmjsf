...
public class FinancialHighlightWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(FinancialHighlightWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			FinancialHighlightDataSource financialHighlightDataSource = new FinancialHighlightDataSource();
			FinancialHighlight financialHighlight = financialHighlightDataSource.get(Integer.parseInt(value));
			FinancialHighlightWrapper financialHighlightWrapper=FinancialHighlightAdapter.wrap(financialHighlight);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((FinancialHighlightWrapper) object).getFinancialHighlight().getFinancialHighlightId());
		} else {
			return null;
		}
	}
