...
public class BalancesheetWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(BalancesheetWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			BalancesheetDataSource balancesheetDataSource = new BalancesheetDataSource();
			Balancesheet balancesheet = balancesheetDataSource.get(Integer.parseInt(value));
			BalancesheetWrapper balancesheetWrapper=BalancesheetAdapter.wrap(balancesheet);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((BalancesheetWrapper) object).getBalancesheet().getBalancesheetId());
		} else {
			return null;
		}
	}
