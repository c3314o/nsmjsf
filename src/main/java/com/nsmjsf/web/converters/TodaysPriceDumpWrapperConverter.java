...
public class TodaysPriceDumpWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(TodaysPriceDumpWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			TodaysPriceDumpDataSource todaysPriceDumpDataSource = new TodaysPriceDumpDataSource();
			TodaysPriceDump todaysPriceDump = todaysPriceDumpDataSource.get(Integer.parseInt(value));
			TodaysPriceDumpWrapper todaysPriceDumpWrapper=TodaysPriceDumpAdapter.wrap(todaysPriceDump);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((TodaysPriceDumpWrapper) object).getTodaysPriceDump().getTodaysPriceDumpId());
		} else {
			return null;
		}
	}
