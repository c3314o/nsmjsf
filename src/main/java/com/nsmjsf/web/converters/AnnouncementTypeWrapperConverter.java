...
public class AnnouncementTypeWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(AnnouncementTypeWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			AnnouncementTypeDataSource announcementTypeDataSource = new AnnouncementTypeDataSource();
			AnnouncementType announcementType = announcementTypeDataSource.get(Integer.parseInt(value));
			AnnouncementTypeWrapper announcementTypeWrapper=AnnouncementTypeAdapter.wrap(announcementType);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((AnnouncementTypeWrapper) object).getAnnouncementType().getAnnouncementTypeId());
		} else {
			return null;
		}
	}
