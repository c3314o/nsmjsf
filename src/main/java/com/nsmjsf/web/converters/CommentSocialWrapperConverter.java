...
public class CommentSocialWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(CommentSocialWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			CommentSocialDataSource commentSocialDataSource = new CommentSocialDataSource();
			CommentSocial commentSocial = commentSocialDataSource.get(Integer
					.parseInt(value));
			CommentSocialWrapper commentSocialWrapper = CommentSocialAdapter
					.wrap(commentSocial);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((CommentSocialWrapper) object)
