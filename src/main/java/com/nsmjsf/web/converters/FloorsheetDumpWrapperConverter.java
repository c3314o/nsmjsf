...
public class FloorsheetDumpWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(FloorsheetDumpWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			FloorsheetDumpDataSource floorsheetDumpDataSource = new FloorsheetDumpDataSource();
			FloorsheetDump floorsheetDump = floorsheetDumpDataSource.get(Integer.parseInt(value));
			FloorsheetDumpWrapper floorsheetDumpWrapper=FloorsheetDumpAdapter.wrap(floorsheetDump);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((FloorsheetDumpWrapper) object).getFloorsheetDump().getFloorsheetDumpId());
		} else {
			return null;
		}
	}
