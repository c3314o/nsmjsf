...
public class CurrencyTypeWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(CurrencyTypeWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			CurrencyTypeDataSource currencyTypeDataSource = new CurrencyTypeDataSource();
			CurrencyType currencyType = currencyTypeDataSource.get(Integer
					.parseInt(value));
			CurrencyTypeWrapper currencyTypeWrapper = CurrencyTypeAdapter
					.wrap(currencyType);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((CurrencyTypeWrapper) object)
					.getCurrencyType().getCurrencyTypeId());
		} else {
			return null;
		}
	}
