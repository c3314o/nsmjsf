...
public class BonusDividendApprovedWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(BonusDividendApprovedWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			BonusDividendApprovedDataSource bonusDividendApprovedDataSource = new BonusDividendApprovedDataSource();
			BonusDividendApproved bonusDividendApproved = bonusDividendApprovedDataSource
					.get(Integer.parseInt(value));
			BonusDividendApprovedWrapper bonusDividendApprovedWrapper = BonusDividendApprovedAdapter
					.wrap(bonusDividendApproved);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((BonusDividendApprovedWrapper) object)
					.getBonusDividendApproved().getBonusDividendApprovedId());
		} else {
			return null;
		}
	}
