...
public class BullionTypeWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(BullionTypeWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			BullionTypeDataSource bullionTypeDataSource = new BullionTypeDataSource();
			BullionType bullionType = bullionTypeDataSource.get(Integer
					.parseInt(value));
			BullionTypeWrapper bullionTypeWrapper = BullionTypeAdapter
					.wrap(bullionType);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((BullionTypeWrapper) object)
