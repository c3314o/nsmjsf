...
public class UserBullionSalesWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(UserBullionSalesWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserBullionSalesDataSource userBullionSalesDataSource = new UserBullionSalesDataSource();
			UserBullionSales userBullionSales = userBullionSalesDataSource
					.get(Integer.parseInt(value));
			UserBullionSalesWrapper userBullionSalesWrapper = UserBullionSalesAdapter
					.wrap(userBullionSales);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserBullionSalesWrapper) object)
					.getUserBullionSales().getUserBullionSalesId());
		} else {
			return null;
		}
	}
