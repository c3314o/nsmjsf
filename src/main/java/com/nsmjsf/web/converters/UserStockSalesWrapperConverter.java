...
public class UserStockSalesWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(UserStockSalesWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserStockSalesDataSource userStockSalesDataSource = new UserStockSalesDataSource();
			UserStockSales userStockSales = userStockSalesDataSource
					.get(Integer.parseInt(value));
			UserStockSalesWrapper userStockSalesWrapper = UserStockSalesAdapter
					.wrap(userStockSales);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserStockSalesWrapper) object)
