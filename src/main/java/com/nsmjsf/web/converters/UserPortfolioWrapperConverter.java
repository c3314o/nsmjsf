...
public class UserPortfolioWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(UserPortfolioWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserPortfolioDataSource userPortfolioDataSource = new UserPortfolioDataSource();
			UserPortfolio userPortfolio = userPortfolioDataSource.get(Integer
					.parseInt(value));
			UserPortfolioWrapper userPortfolioWrapper = UserPortfolioAdapter
					.wrap(userPortfolio);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserPortfolioWrapper) object)
