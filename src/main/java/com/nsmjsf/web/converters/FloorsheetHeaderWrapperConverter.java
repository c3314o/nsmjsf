...
public class FloorsheetHeaderWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(FloorsheetHeaderWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			FloorsheetHeaderDataSource floorsheetHeaderDataSource = new FloorsheetHeaderDataSource();
			FloorsheetHeader floorsheetHeader = floorsheetHeaderDataSource
					.get(Integer.parseInt(value));
			FloorsheetHeaderWrapper floorsheetHeaderWrapper = FloorsheetHeaderAdapter
					.wrap(floorsheetHeader);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((FloorsheetHeaderWrapper) object)
					.getFloorsheetHeader().getFloorsheetHeaderId());
		} else {
			return null;
		}
	}
