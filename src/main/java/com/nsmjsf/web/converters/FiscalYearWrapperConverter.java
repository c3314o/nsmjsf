...
public class FiscalYearWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(FiscalYearWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			FiscalYearDataSource fiscalYearDataSource = new FiscalYearDataSource();
			FiscalYear fiscalYear = fiscalYearDataSource.get(Integer
					.parseInt(value));
			FiscalYearWrapper fiscalYearWrapper = FiscalYearAdapter
					.wrap(fiscalYear);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((FiscalYearWrapper) object).getFiscalYear()
					.getFiscalYearId());
		} else {
			return null;
		}
