...
public class FloorsheetWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(FloorsheetWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			FloorsheetDataSource floorsheetDataSource = new FloorsheetDataSource();
			Floorsheet floorsheet = floorsheetDataSource.get(Integer
					.parseInt(value));
			FloorsheetWrapper floorsheetWrapper = FloorsheetAdapter
					.wrap(floorsheet);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((FloorsheetWrapper) object).getFloorsheet()
					.getFloorsheetId());
		} else {
			return null;
		}
