...
public class CurrencyRateWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(CurrencyRateWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			CurrencyRateDataSource currencyRateDataSource = new CurrencyRateDataSource();
			CurrencyRate currencyRate = currencyRateDataSource.get(Integer.parseInt(value));
			CurrencyRateWrapper currencyRateWrapper=CurrencyRateAdapter.wrap(currencyRate);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((CurrencyRateWrapper) object).getCurrencyRate().getCurrencyRateId());
		} else {
			return null;
		}
	}
