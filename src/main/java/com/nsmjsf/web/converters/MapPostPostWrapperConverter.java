...
public class MapPostPostWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(MapPostPostWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			MapPostPostDataSource mapPostPostDataSource = new MapPostPostDataSource();
			MapPostPost mapPostPost = mapPostPostDataSource.get(Integer.parseInt(value));
			MapPostPostWrapper mapPostPostWrapper=MapPostPostAdapter.wrap(mapPostPost);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((MapPostPostWrapper) object).getMapPostPost().getMapPostPostId());
		} else {
			return null;
		}
	}
