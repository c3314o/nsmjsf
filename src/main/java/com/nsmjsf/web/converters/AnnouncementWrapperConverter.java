...
public class AnnouncementWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(AnnouncementWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			AnnouncementDataSource announcementDataSource = new AnnouncementDataSource();
			Announcement announcement = announcementDataSource.get(Integer
					.parseInt(value));
			AnnouncementWrapper announcementWrapper = AnnouncementAdapter
					.wrap(announcement);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((AnnouncementWrapper) object)
					.getAnnouncement().getAnnouncementId());
		} else {
			return null;
		}
	}
