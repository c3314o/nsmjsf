...
public class BasePriceWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(BasePriceWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			BasePriceDataSource basePriceDataSource = new BasePriceDataSource();
			BasePrice basePrice = basePriceDataSource.get(Integer.parseInt(value));
			BasePriceWrapper basePriceWrapper=BasePriceAdapter.wrap(basePrice);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((BasePriceWrapper) object).getBasePrice().getBasePriceId());
		} else {
			return null;
		}
	}
