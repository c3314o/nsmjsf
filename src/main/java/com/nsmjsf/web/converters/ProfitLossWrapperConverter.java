...
public class ProfitLossWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(ProfitLossWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			ProfitLossDataSource profitLossDataSource = new ProfitLossDataSource();
			ProfitLoss profitLoss = profitLossDataSource.get(Integer.parseInt(value));
			ProfitLossWrapper profitLossWrapper=ProfitLossAdapter.wrap(profitLoss);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((ProfitLossWrapper) object).getProfitLoss().getProfitLossId());
		} else {
			return null;
		}
	}
