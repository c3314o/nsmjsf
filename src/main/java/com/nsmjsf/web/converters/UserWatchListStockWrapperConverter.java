...
public class UserWatchListStockWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(UserWatchListStockWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserWatchListStockDataSource userWatchListStockDataSource = new UserWatchListStockDataSource();
			UserWatchListStock userWatchListStock = userWatchListStockDataSource.get(Integer.parseInt(value));
			UserWatchListStockWrapper userWatchListStockWrapper=UserWatchListStockAdapter.wrap(userWatchListStock);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserWatchListStockWrapper) object).getUserWatchListStock().getUserWatchListStockId());
		} else {
			return null;
		}
	}
