...
public class CompanyBranchWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(CompanyBranchWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			CompanyBranchDataSource companyBranchDataSource = new CompanyBranchDataSource();
			CompanyBranch companyBranch = companyBranchDataSource.get(Integer.parseInt(value));
			CompanyBranchWrapper companyBranchWrapper=CompanyBranchAdapter.wrap(companyBranch);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((CompanyBranchWrapper) object).getCompanyBranch().getCompanyBranchId());
		} else {
			return null;
		}
	}
