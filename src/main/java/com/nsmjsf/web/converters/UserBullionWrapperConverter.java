...
public class UserBullionWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(UserBullionWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			UserBullionDataSource userBullionDataSource = new UserBullionDataSource();
			UserBullion userBullion = userBullionDataSource.get(Integer.parseInt(value));
			UserBullionWrapper userBullionWrapper=UserBullionAdapter.wrap(userBullion);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((UserBullionWrapper) object).getUserBullion().getUserBullionId());
		} else {
			return null;
		}
	}
