...
public class LatestPriceWrapperConverter implements Converter{
	private static final Log log = LogFactory.getLog(LatestPriceWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			LatestPriceDataSource latestPriceDataSource = new LatestPriceDataSource();
			LatestPrice latestPrice = latestPriceDataSource.get(Integer.parseInt(value));
			LatestPriceWrapper latestPriceWrapper=LatestPriceAdapter.wrap(latestPrice);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((LatestPriceWrapper) object).getLatestPrice().getLatestPriceId());
		} else {
			return null;
		}
	}
