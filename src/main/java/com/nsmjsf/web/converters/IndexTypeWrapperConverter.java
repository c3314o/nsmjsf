...
public class IndexTypeWrapperConverter implements Converter {
	private static final Log log = LogFactory
			.getLog(IndexTypeWrapperConverter.class);

	@Override
	public Object getAsObject(FacesContext fc, UIComponent arg1, String value) {
		if (value != null && value.trim().length() > 0) {
			IndexTypeDataSource indexTypeDataSource = new IndexTypeDataSource();
			IndexType indexType = indexTypeDataSource.get(Integer
					.parseInt(value));
			IndexTypeWrapper indexTypeWrapper = IndexTypeAdapter
					.wrap(indexType);
	...
	public String getAsString(FacesContext fc, UIComponent arg1, Object object) {
		if (object != null) {
			return String.valueOf(((IndexTypeWrapper) object).getIndexType()
					.getIndexTypeId());
		} else {
			return null;
		}
